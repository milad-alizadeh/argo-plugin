...
| 14 | done | (pending commit) | Live-file verification against real Argo Figma files (project `CLEHEoqvJlRti3dCCfOytS`, kit copy `4lPUPl8OUan4i90Bc2ZMXe` — both currently empty besides a Cover page; built temporary fixtures, reverted after). Confirmed `unbound-fill` correctly passes/fails. **Confirmed the council's suspicion**: `non-semantic-binding` was vacuous for the local-Primitives-vs-Semantic distinction — `isSemantic` only checked "has *some* variableCollectionId", true for ANY local variable including Primitives. Fixed: now compares the bound variable's actual collection *name* against `SEMANTIC_COLLECTION_NAME`. Also caught mid-fix: spreading a live `Variable` object (`{...variable}`) silently drops `remote`/`key`/`variableCollectionId` (prototype getters, not own enumerable props) — walker now marshals those three fields explicitly instead of spreading. Re-verified live post-fix: Primitive-bound now correctly flagged, Semantic-bound still passes. Fixtures fully reverted (get_metadata diffed clean). Not tested live: `kit-patches-conformance` (still gated on the documented `collectModifiedKitCopyNodes()` TODO) and cross-file kit-sourced (`remote: true`) bindings, which would require publishing the kit-copy file as a library — out of scope for a temporary fixture; deferred to argo-v2 Phase B's real sync spike, consistent with the plan's own carried-forward gap. |
