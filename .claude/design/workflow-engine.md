# Argo workflow engine — ground-up redesign

Status: design settled via grill (2026-07-09). Next: `argo:planner` for the implementation plan.

## Problem

Argo's workflows (design pipeline, gated builds) are implemented as bespoke prose-heavy
skills, hand-written hooks (design-guard, session-guard), and ad-hoc state (receipt files,
progress docs). Every new workflow means new hook code; enforcement lives in skill text
...
Argo is a **passive spine over an existing interactive agent**: workflow specs +
...
loop: workflows run start-to-finish autonomously; review happens out-of-band.
...
├── adapter-claude/  @argohq/adapter-claude  hooks, session lifecycle, tool-call classifier
├── pack-product/    @argohq/pack-product    prd-create workflow, brief-check, write-prd/grill skills' logic
├── pack-design/     @argohq/pack-design     six design workflows, design-rules-check, fresh-eyes-review, registry
└── pack-code/       @argohq/pack-code       six code workflows, tests-pass, policies, reporters, code-review
...
packs: workflows can't start, skills inert, rules never installed. A code-only project
never sees Figma; a design-only project never sees TDD.

`.argo/config.json` is the SINGLE config surface: packs, `noWorkflow`,
...
`.argo/config.json`, stage specs (`defineWorkflow` output), and gate contracts
...
`argo workflow start` — it never silently disables a gate.

One general engine rule for cross-pack handoffs: a workflow whose terminal stage hands
off to a disabled pack (design-to-code → pack-code's screen-implement) is refused at
`argo workflow start` with a config error naming the missing pack, never failing
...
spec branching. The one engine mechanism beyond the flat list is cross-workflow
dependency resolution: a stage that requires another component's or workflow's output
triggers the engine to spawn the prerequisite workflow first ("Missing components?" is
this, not stage branching), plus an early-`done` exit when a guard finds the work
already exists ("Exists? Stop.").

Because specs are data, core ships `argo workflow diagram <name> [--all]`: renders a
...
- No human gate. Workflow ends `done` when the last gate passes; review is out-of-band.
...
- One core action kind, `git-history-mutation` (reset, amend, rebase, checkout of
  ...
  code workflows needs it; history rewriting would otherwise let an agent erase the
...
- ONE generic PreToolUse hook (matcher `*`): read active instance from state (session-
  cached) → no active workflow ⇒ config decides (`noWorkflow: "allow" | "deny-edits"`
  in `.argo/config.json`; default allow — argo never blocks a human tinkering; deny for
  autonomous projects: no coding outside a workflow, hook coaches "start one") →
...
- `workflows/<key>.json` — instance: `{ workflow, target, stage, status, attempts: [
...
- Repo commits only durable artifacts. `argo workflow adopt` re-derives stage from
  ...
  `argo workflow status` instead of silently advancing.

CLI verbs: `argo workflow start <name> --target <t> / status / advance / adopt`.

### Routing (who picks the workflow)

Explicit start is the only entry point (state, permissions, gates hang off it). The
user may start one directly; otherwise the agent triages the request to a workflow via
a one-paragraph plugin instruction (a casual "fix this bug" auto-starts bug-fix).
Misrouting is visible in `argo workflow status` and cheap to correct. With
`noWorkflow: "deny-edits"`, everything code-touching goes through a workflow — the
...
gate invocations, no state writes (may read `argo workflow status`), no enforcement
...
workflows (design-screen serves screen-create's build and screen-edit's edit). Slash
commands become `argo workflow start` wrappers. Agents shrink to persona + toolset.
...
7. **land**: explicit-start only, never triaged into (outward-facing). Requires the
   branch's workflows to be `done` — the engine mechanically refuses to ship unreviewed
...
|---|---|
| defineWorkflow/engine/state/CLI | core (new, small) |
...
| integrator agent's push/PR duties | pack-code `land` workflow (agent persona may remain) |
| templates/rules | plugin, installed per enabled pack by argo init |

## Phasing

1. core + adapter-claude + pack-design on the engine (design workflows prove it — they
   are the active pain).
