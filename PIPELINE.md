...
| 5 | **Hi-fi** | `design-screen` (`design-component` for one component) | brief + PRD's ASCII wireframe | hi-fi Figma, built component-first | hard **design-rules audit** + deterministic instance-presence check; **`design-verifier`** (advisory, adversarial, given-only) rules every PRD requirement present |
