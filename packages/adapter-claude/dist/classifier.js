...
const GIT_HISTORY_MUTATION_PATTERNS = [
    /\bgit\s+reset\b/,
    /\bgit\s+commit\b[^\n]*--amend\b/,
    /\bgit\s+rebase\b/,
...
export function classifyFigmaScript(script) {
...
function extractFigmaScript(toolInput) {
    if (toolInput && typeof toolInput === 'object') {
        const candidate = toolInput.script ??
            toolInput.code ??
            toolInput.command;
...
export function classifyAction(toolName, toolInput) {
    ...
    if (toolName === 'mcp__plugin_figma_figma__use_figma') {
        ...
        if (script !== undefined)
            return classifyFigmaScript(script);
        return UNCLASSIFIED;
    }
