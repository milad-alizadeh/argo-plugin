...
export function runPermissionHook(
  ...
  if (!stage) {
    return deny(
      `active workflow "${instance.workflow}" is at unknown stage "${instance.stage}" — denying (fail closed)`
    )
  }
  ...
  return deny(
    `stage "${instance.stage}" of workflow "${instance.workflow}" does not allow "${kind}"${pathSuffix} — ` +
