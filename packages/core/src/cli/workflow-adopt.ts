...
import { deriveInstanceKey, writeInstance, type HistoryEntry, type StateOptions, type WorkflowInstance } from '../state.js'
import { GateNotFoundError, WorkflowNotFoundError } from './errors.js'
 ...
 */
export async function workflowAdopt(input: WorkflowAdoptInput, opts: WorkflowAdoptOptions = {}): Promise<WorkflowInstance> {
  const spec = getWorkflow(input.name)
  if (!spec) throw new WorkflowNotFoundError(input.name)
