...
import { getWorkflow, type WorkflowSpec } from '../spec.js'
import {
  readInstance,
  recordAttempt,
  recordHistory,
 ...
 */
export async function workflowAdvance(key: string, opts: WorkflowAdvanceOptions = {}): Promise<WorkflowInstance> {
  ...
  if (!spec) throw new WorkflowNotFoundError(instance.workflow)

  const stageIndex = spec.stages.findIndex((s) => s.name === instance.stage)
  const stageSpec = spec.stages[stageIndex]
  if (!stageSpec) throw new StageNotFoundError(instance.stage, instance.workflow)
