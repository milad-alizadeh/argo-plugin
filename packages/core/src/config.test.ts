...
describe('readConfig', () => {
  ...
  it('falls back to defaults for keys missing from a partial file', () => {
    const argoDir = join(cwd, '.argo')
    mkdirSync(argoDir, { recursive: true })
    writeFileSync(join(argoDir, 'config.json'), JSON.stringify({ noWorkflow: 'deny-edits' }))
  ...
  it('reads live per call — a later edit is picked up without caching', () => {
    ...
    writeFileSync(path, JSON.stringify({ noWorkflow: 'allow' }))
    expect(readConfig(cwd).noWorkflow).toBe('allow')

    writeFileSync(path, JSON.stringify({ noWorkflow: 'deny-edits' }))
    expect(readConfig(cwd).noWorkflow).toBe('deny-edits')
  })
})

describe('assertPackAvailable', () => {
  function makeConfig(packs: Record<string, boolean>): ArgoConfig {
    return { packs, noWorkflow: 'allow', testDiscipline: undefined, land: undefined }
  }
