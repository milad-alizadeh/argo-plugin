...
describe('isProtectedPath', () => {
  it('matches ~/.argo/state/** (absolute home-anchored path)', () => {
    expect(isProtectedPath(join(homedir(), '.argo', 'state', 'proj-id', 'workflows', 'key.json'))).toBe(true)
  })

  it('matches ~/.argo/state/** (literal ~ form)', () => {
    expect(isProtectedPath('~/.argo/state/proj-id/workflows/key.json')).toBe(true)
  })
