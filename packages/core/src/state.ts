...
export function deriveInstanceKey(workflow: string, target: string): string {
...
export function readInstance(key: string, opts: StateOptions = {}): WorkflowInstance | null {
  const path = instancePath(key, opts)
  if (!existsSync(path)) return null
  try {
    return JSON.parse(readFileSync(path, 'utf8')) as WorkflowInstance
  } catch {
    return null
  }
}
...
export function writeInstance(key: string, instance: WorkflowInstance, opts: StateOptions = {}): void {
  const path = instancePath(key, opts)
...
export function recordAttempt(key: string, attempt: Attempt, opts: StateOptions = {}): WorkflowInstance {
  const instance = readInstance(key, opts)
  if (!instance) throw new Error(`no instance for key "${key}" — writeInstance first`)
  const updated: WorkflowInstance = { ...instance, attempts: [...instance.attempts, attempt] }
...
export function recordHistory(key: string, entry: HistoryEntry, opts: StateOptions = {}): WorkflowInstance {
  const instance = readInstance(key, opts)
  if (!instance) throw new Error(`no instance for key "${key}" — writeInstance first`)
  const updated: WorkflowInstance = { ...instance, history: [...instance.history, entry] }
  writeInstance(key, updated, opts)
  return updated
}
...
export function getActiveInstance(opts: StateOptions = {}): WorkflowInstance | null {
