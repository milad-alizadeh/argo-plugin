...
export function installedKitVersion() {
    const pkgPath = fileURLToPath(new URL('../../package.json', import.meta.url));
    return JSON.parse(readFileSync(pkgPath, 'utf8')).version;
}
export function runDoctor({ pluginRoot, kitVersion = installedKitVersion() } = {}) {
    if (!pluginRoot) {
    ...
    if (!declared)
        return { ok: false, reason: 'doctor: plugin.json declares no designLibrary major.minor (default-deny)' };
    if (!installed)
        return { ok: false, reason: `doctor: installed kit version "${kitVersion}" is not parseable (default-deny)` };
    if (declared === installed)
        return { ok: true, declared, installed };
