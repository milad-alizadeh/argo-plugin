...
export function runGraphRefresh({ cwd = process.cwd(), env = process.env } = {}) {
    const sh = (cmd, args, opts = {}) => spawnSync(cmd, args, { cwd, encoding: 'utf8', env, ...opts });
    ...
    if (branch !== 'main')
        return { skipped: 'not-on-main', branch };
    const gitDir = sh('git', ['rev-parse', '--git-dir']).stdout?.trim() ?? '';
    if (gitDir.includes('/worktrees/'))
        return { skipped: 'in-worktree' };
    ...
    for (const ws of workspaces) {
        ...
        const run = (args, extraEnv = {}) => spawnSync('graphify', args, { cwd: wsDir, encoding: 'utf8', env: { ...env, ...extraEnv }, stdio: 'inherit' });
        run(['update', '.', '--force'], { PYTHONHASHSEED: '0' });
        const label = run(['label', '.', '--missing-only', '--backend=claude-cli']);
        if (label.status !== 0)
            labelDegraded.push(ws);
    ...
    const gitAtRoot = (args, extraEnv = {}) => spawnSync('git', args, { cwd: repoRoot, encoding: 'utf8', env: { ...env, ...extraEnv } });
    for (const spec of GRAPH_PATHSPECS)
        gitAtRoot(['add', '--', spec]);
...
function pruneDatedBackups(graphifyOut) {
    ...
    for (const entry of readdirSync(graphifyOut, { withFileTypes: true })) {
        if (!entry.isDirectory() || !DATED_BACKUP.test(entry.name))
            continue;
...
//# sourceMappingURL=graph-refresh.js.map