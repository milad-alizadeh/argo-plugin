 ...
 */
export function convertLineHeight(figmaValue, unit) {
    if (unit === 'PERCENT')
        return String(figmaValue / 100);
    if (unit === 'PIXELS')
...
export function convertLetterSpacing(percentValue, fontSize) {
 ...
 */
export function resolveBoxModel(layoutSizing, { hugTolerance = DEFAULT_HUG_TOLERANCE } = {}) {
    if (layoutSizing === 'FIXED')
        return { checkType: 'fixed', tolerance: 0 };
    if (layoutSizing === 'HUG')
        return { checkType: 'hug', tolerance: hugTolerance };
    if (layoutSizing === 'FILL')
        return { checkType: 'skip', tolerance: null };
