import { unboundFillViolations, unboundStrokeViolations, unboundRadiusViolation, unboundTypeViolation, missingAutoLayoutViolation, handDrawnIconViolation, kitInstanceOverrideViolation, detachedInstanceViolation, nonSemanticNameViolation, variantNamingViolations, modeCopyViolations, implicitLineHeightViolation, storyUrlScopeViolation, gapPaddingSpacingViolations, isNamedAuditTarget, isWireframePageName, strokeScaleViolation, possibleGateFalsePositiveTag, compositeRegionNamingViolation } from './tier0-rules.js';
async function auditNode(node, { hard, spacingScale, semanticModes, semanticCollectionName, insideInstance = false, compositeNames = [], runRecipeTier0Checks }) {
    ...
    const overriddenFields = node.type === 'INSTANCE' ? (node.overrides ?? []).flatMap((o) => o.overriddenFields ?? []) : [];
    ...
    for (const v of unboundFillViolations(nodeCtx))
        report(v.rule, v.detail);
    for (const v of unboundStrokeViolations(nodeCtx))
        report(v.rule, v.detail);
    const radius = unboundRadiusViolation(nodeCtx);
    if (radius)
        report(radius.rule, radius.detail);
    const type = unboundTypeViolation(nodeCtx);
    if (type)
        report(type.rule, type.detail);
    const autoLayout = missingAutoLayoutViolation(nodeCtx);
    if (autoLayout)
        report(autoLayout.rule, autoLayout.detail);
    const handDrawnIcon = handDrawnIconViolation({ type: node.type, insideInstance });
    if (handDrawnIcon)
        report(handDrawnIcon.rule, handDrawnIcon.detail);
    ...
    if (node.type === 'INSTANCE') {
        ...
        if (detached)
            report(detached.rule, detached.detail);
        ...
        if (kitOverride)
            report(kitOverride.rule, kitOverride.detail);
        if (main?.remote) {
    ...
    if (nonSemanticName)
        report(nonSemanticName.rule, nonSemanticName.detail);
    ...
    for (const v of variantNamingViolations(node))
        report(v.rule, v.detail);
    ...
    if (lineHeight)
        report(lineHeight.rule, lineHeight.detail);
    ...
    if (node.type === 'COMPONENT') {
        ...
        if (storyUrl) {
            ...
            if (storyScope)
                report(storyScope.rule, storyScope.detail);
        }
    }
...
function marshalIconStrokeScale(node, main) {
    ...
    if ((main.children ?? []).length !== 1 || vectorChildren.length !== 1)
        return null;
    ...
    if (typeof baseStrokeWeight !== 'number' || typeof resolvedStrokeWeight !== 'number')
        return null;
    if (typeof main.width !== 'number' || typeof node.width !== 'number')
        return null;
...
async function marshalGapPaddingField(node, field) {
    ...
    if (!bound?.id)
        return { field, value, bound: false };
...
async function walk(node, opts, out) {
    ...
    if ('children' in node) {
        const childOpts = node.type === 'INSTANCE' && !opts.insideInstance ? { ...opts, insideInstance: true } : opts;
        for (const child of node.children)
            await walk(child, childOpts, out);
    }
}
/**
...
export async function runTier0Audit(options = {}) {
    const { componentNames, compositeNames = [], semanticCollectionName = 'Semantic', runRecipeTier0Checks, runKitPatchesConformance } = options;
    const violations = [];
    ...
    if (componentNames?.length) {
        ...
        for (const name of componentNames) {
            ...
            for (const match of matches) {
                if (isWireframePageName(findOwningPage(match)?.name ?? ''))
                    continue;
        ...
        for (const page of figma.root.children) {
            ...
            // wording.
            if (isWireframePageName(page.name))
                continue;
...
async function collectPrimitivesSpacingScale() {
    ...
    for (const variableId of primitives.variableIds) {
        ...
        if (!variable)
            continue;
        if (!variable.scopes.includes('GAP') && !variable.scopes.includes('WIDTH_HEIGHT'))
            continue;
...
//# sourceMappingURL=tier0-audit.js.map