...
export function unboundRadiusViolation(node) {
    ...
    if (boundUniform || boundPerCorner)
        return null;
...
export function kitInstanceOverrideViolation(node) {
    if (node.type !== 'INSTANCE' || !node.isRemoteInstance)
        return null;
...
export function possibleGateFalsePositiveTag(node) {
    if (!node.isRemoteInstance && !node.insideInstance)
        return false;
...
export function compositeRegionNamingViolation(node, compositeNames) {
    if (node.type !== 'FRAME')
        return null;
    if (!(compositeNames ?? []).includes(node.name))
        return null;
    return {
...
//# sourceMappingURL=tier0-rules.js.map