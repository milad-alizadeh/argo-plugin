...
if (hook?.hook_event_name !== 'Stop' && hook?.hook_event_name !== 'SubagentStop')
    process.exit(0);
const cwd = hook?.cwd;
...
if (backgroundTasks.length > 0)
    process.exit(0); // in-flight subagent/workflow may still write Figma changes — defer, don't block
...
if (sessionId) {
    ...
    if (sessionWriteCount === 0)
        process.exit(0); // this session made zero Figma writes — nothing owed
}
...
process.exit(0);
//# sourceMappingURL=design-guard-stop.js.map