...
if (typeof proof.recordedAt !== 'number')
    block('receipt has no recordedAt timestamp');
...
const stagesInAddSegment = (seg) => /\bgit\b[^\n]*\badd\b/.test(seg) &&
...
const commandStagesTestFile = command.split(/[;\n]+/).some((statement) => {
    ...
    if (/\|\|/.test(before))
        return false; // || between add and commit: commit runs on FAILURE
...
process.exit(0);
//# sourceMappingURL=red-proof-gate.js.map