...
if (!existsSync(markerPath))
    process.exit(0); // not a gated build — inert
...
if (!receiptPath)
    block('no launch evidence — the app was never launched (no receipt found)');
let receipt;
try {
...
if (!wellFormed)
    block('receipt is malformed / missing required fields (default-deny)');
if (receipt.exitCode === null)
    block('launch is still running (exitCode:null) — refusing to pass a half-run');
if (receipt.exitCode !== 0)
    block(`launch exited non-zero (exitCode ${receipt.exitCode})`);
if (!receipt.exercised)
    block('launched but never exercised — no observable evidence through the real surface');
...
if (age < 0 || age > MAX_RECEIPT_AGE_MS)
    block(`receipt timestamp out of range (${Math.round(age / 1000)}s) — re-launch required`);
