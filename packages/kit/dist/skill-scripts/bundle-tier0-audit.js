...
function makeExportForBundling(source) {
    ...
    if (!BARE_IDENTIFIER.test(lastLine)) {
        throw new Error(`bundleTier0Audit: expected the entry source to end with a bare identifier (its sandbox completion value), found "${lastLine}"`);
    }
...
export function bundleTier0Audit(entrySource, { cwd, maxChars = 50000 }) {
    if (!cwd)
        throw new Error('bundleTier0Audit: cwd is required (@argohq/kit resolves from its node_modules)');
...
export function bundleTier0AuditForRecipe({ cwd, recipe = null, outPath, maxChars = 50000 }) {
    if (!cwd)
        throw new Error('bundleTier0AuditForRecipe: cwd is required');
...
//# sourceMappingURL=bundle-tier0-audit.js.map