...
export function buildKitInventory({ components = [], icons }, { kitLibraryFileKey, kitSourceVersion, now = Date.now() } = {}) {
    return {
        _usage: 'Kit components are used AS-IS via base instances; prefer them over custom builds. Building custom when a kit component (or alias) matches requires a design/waivers.json entry naming the kit candidate and a concrete reason it is insufficient.',
        kitLibraryFileKey,
        kitSourceVersion,
...
if (import.meta.url === `file://${process.argv[1]}`) {
    ...
    if (recordIndex === -1 || outIndex === -1) {
        console.error('capture-kit-inventory: usage: argo design capture-kit-inventory --record ' +
...
//# sourceMappingURL=capture-kit-inventory.js.map