 ...
 */
function findDesignBlock(cwd) {
    const found = findArgoJson(cwd);
    const entries = Object.entries(found?.config?.design ?? {});
    if (entries.length === 0)
        return null;
    const resolvedCwd = resolve(cwd);
    const matched = entries.find(([, block]) => resolve(found.repoRoot, block?.root ?? '.') === resolvedCwd);
    if (matched)
        return matched[1];
...
//# sourceMappingURL=prepare-tier0-audit-options.js.map