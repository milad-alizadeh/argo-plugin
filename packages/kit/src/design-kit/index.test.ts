...
import * as figmaDesignKit from './index.js'
...
describe('figma-design-kit package entrypoint', () => {
  it('re-exports every mechanism module the package.json exports field promises', () => {
    ...
    expect(typeof figmaDesignKit.diffVariableDefs).toBe('function')
    expect(typeof figmaDesignKit.classifyNodeDrift).toBe('function')
    expect(typeof figmaDesignKit.classifyStaleness).toBe('function')
    expect(typeof figmaDesignKit.diffVariantShape).toBe('function')
  })
  ...
  it('does not export the deleted kit-subscription schemas (kit.lock / kit-patches are gone)', () => {
    ...
    expect((figmaDesignKit as any).WaiverSchema).toBeUndefined()
  })
})
