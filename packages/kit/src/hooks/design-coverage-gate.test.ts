...
import { evaluateCoverageReceipt } from './design-coverage-gate.js'
...
// sibling .ts source. Requires `bun run build` to have produced a current
// packages/kit/dist/ before this test runs.
const GATE = fileURLToPath(new URL('../../dist/hooks/design-coverage-gate.js', import.meta.url))
...
const commitInput = (cwd: string, command = 'git commit -m "feat: screen"') =>
...
function stageScreenWork(repo: string, appRoot: string, screen: string, { figmaFileVersion = '42' } = {}) {
...
function writeCoverageReceipt(repo: string, appRoot: string, screen: string, overrides: Record<string, unknown> = {}) {
  writeFileSync(
