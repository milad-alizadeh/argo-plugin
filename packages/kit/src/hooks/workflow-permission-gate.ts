#!/usr/bin/env node
/**
 * Workflow permission gate — the generic PreToolUse hook body (design doc /
 * `.claude/plans/workflow-engine-phase1.md`, Slice 8 step 23) wired into kit's
 ...
 * `hooks/hooks.json` — because `@argohq/adapter-claude`'s `runPermissionHook`
 * needs to see Bash, Edit/Write, and Figma calls alike to enforce a workflow
 ...
 * concept of "the" active workflow for a project (an instance is addressed by
 * `deriveInstanceKey(workflow, target)`, and a generic tool call carries
 * neither). `workflow-start`/`workflow-adopt` now write a project-scoped
 ...
 */
import { runPermissionHook, type HookInput } from '@argohq/adapter-claude'
import { getActiveInstance, readConfig } from '@argohq/core'
...
process.stderr.write(`Workflow permission gate: BLOCKED — ${decision.reason}\n`)
process.exit(2)
