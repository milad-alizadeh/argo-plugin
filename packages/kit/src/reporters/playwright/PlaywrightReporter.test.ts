...
function fakeTest({ id = 't1', title = 'shows roster', file = FILE_A, project = 'chromium', suites = [] as string[] } = {}) {
  return {
    id,
    title,
...
describe('tdd-guard-playwright reporter — writes the tdd-guard test.json contract', () => {
  let root: string, reporter: PlaywrightReporter
  ...
  async function run(events: [any, any][], endStatus = 'failed') {
  ...
  it('multiple projects: same title reported per-project, distinguishable via fullName', async () => {
    ...
    const tests = written().testModules[0].tests
    expect(tests.map((t: any) => t.fullName)).toEqual([
      'chromium > shows roster',
      'electron > shows roster',
    ])
    expect(tests.map((t: any) => t.state)).toEqual(['passed', 'failed'])
  })
  ...
  it('groups tests by file into separate testModules', async () => {
    ...
    expect(written().testModules.map((m: any) => m.moduleId)).toEqual([FILE_A, FILE_B])
  })
  ...
  it('accepts an injected storage (sibling-reporter convention)', async () => {
    const saved: string[] = []
    const custom = new PlaywrightReporter({ storage: { saveTest: async (c: string) => { saved.push(c) } } })
