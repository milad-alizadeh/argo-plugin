...
export function buildKitCorpus(
  { pristine, inverse, semanticModes = [] }: { pristine: unknown; inverse: unknown; semanticModes?: string[] },
  { capturedFrom, now = Date.now() }: { capturedFrom?: string; now?: number } = {}
) {
  return {
