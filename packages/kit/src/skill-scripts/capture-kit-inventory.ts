...
function stripVolatileFields(component: Record<string, any>): Record<string, any> {
...
export function buildKitInventory(
  { components = [], icons }: { components?: Record<string, any>[]; icons?: unknown },
  { kitLibraryFileKey, kitSourceVersion, now = Date.now() }: { kitLibraryFileKey?: string; kitSourceVersion?: string; now?: number } = {}
) {
  return {
    _usage:
