 ...
 * the @argohq/kit/walkers factories, never walker bodies, so a kit upgrade
...
describe('runEmitShims', () => {
  it('single-repo: emits both shims for the armed "." block with defaults derived from componentsPath', () => {
    ...
    expect(specDiff).toContain("import { runSpecDiffWalker } from '@argohq/kit/walkers'")
    ...
    expect(vrt).toContain("import { runVrtWalker } from '@argohq/kit/walkers'")
