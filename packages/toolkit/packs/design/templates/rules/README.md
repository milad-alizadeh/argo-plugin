...
when the design pack is enabled (playbook-engine-phase1.md Slice 12, step 36).
...
what's already covered by `design-rules-check`'s design-rules logic — most of the
skill prose was either narrative technique (kept in `craft/`) or sequencing/
gate-invocation (now the playbook engine's job, stripped entirely).
