...
describe('playbookAdopt', () => {
  let stateRoot: string
  let cwd: string

  beforeEach(() => {
    stateRoot = mkdtempSync(join(tmpdir(), 'argo-playbook-adopt-state-'))
    cwd = mkdtempSync(join(tmpdir(), 'argo-playbook-adopt-cwd-'))
  })
  ...
  it('records verified: false and stops at a gate that signals it cannot be safely re-run', async () => {
    ...
    const playbookName = `adopt-unrerunnable-${Math.random()}`
    registerPlaybook(
      definePlaybook({
        name: playbookName,
        stages: [
    ...
    const instance = await playbookAdopt({ name: playbookName, target: 'fixture-screen' }, { cwd, stateRoot })
  ...
  it('advances through every contiguously-passing stage to done', async () => {
    ...
    const playbookName = `adopt-happy-${Math.random()}`
    registerPlaybook(
      definePlaybook({
        name: playbookName,
        stages: [
    ...
    const instance = await playbookAdopt({ name: playbookName, target: 'fixture-screen' }, { cwd, stateRoot })
    const key = deriveInstanceKey(playbookName, 'fixture-screen')
