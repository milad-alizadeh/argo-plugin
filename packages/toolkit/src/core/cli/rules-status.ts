...
/**
 * Advisory drift report between installed rules' recorded provenance and
 * their CURRENT plugin template content. `templates` is the caller-supplied
 * {filename: content} map — reading files off disk is the bin wrapper's job
 * so this stays a pure comparison over `readConfig`'s output. Keys `current`
 * as `.claude/rules/<filename>` to match the installed-path keys `provenance`
 * is recorded under.
 */
