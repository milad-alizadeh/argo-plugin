...
/** Mirrors design-rules.ts's variantNamingViolations' read of the same node shape. */
 ...
 * `audit-clean` — code-owned nodes are design-rules exempt (a screenshot can't
 ...
 * alone, never hand-maintained. Same word-boundary match the live design-rules audit
 * uses (`design-rules-audit.ts`'s `hasScreenAnnotation`).
 */
 ...
 * registered screen's own artboard false positives are design-rules exempt, so
