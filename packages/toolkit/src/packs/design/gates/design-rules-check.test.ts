...
import type { GateInput } from '../../../core/index.js'
...
describe('design-rules-check gate', () => {
  it('fails on a fixture with a known design-rules violation (non-semantic binding)', async () => {
  ...
  it("evidence points at bundle-design-rules-audit's own receipt (bundle path + content hash)", async () => {
    const gate = createDesignRulesCheckGate({
      readFigma: async () => ({ bindings: [] }),
      cwd: process.cwd()
    })
    ...
    expect(verdict.evidence.some((e) => e.startsWith('design-rules-audit-bundle:'))).toBe(true)
    expect(verdict.evidence.some((e) => e.startsWith('design-rules-audit-hash:'))).toBe(true)
  })
