...
import { definePlaybook, PackUnavailableError, renderPlaybookDiagram, playbookStart } from '../../../core/index.js'
...
describe('designToCodeSpec', () => {
  it('is accepted by definePlaybook without validation errors', () => {
    expect(() => definePlaybook(designToCodeSpec)).not.toThrow()
  })
  ...
  it('renders with NO human-gate (decision diamond) nodes', () => {
    const diagram = renderPlaybookDiagram(designToCodeSpec)
  ...
  describe('playbookStart refusal (Slice 4 assertPackAvailable, via Slice 5 playbookStart)', () => {
    ...
    it('throws PackUnavailableError naming pack-code when pack-code is disabled/absent', () => {
      ...
      expect(() => playbookStart({ name: 'design-to-code', target: 'fixture-screen' }, { cwd, stateRoot })).toThrow(PackUnavailableError)
      expect(() => playbookStart({ name: 'design-to-code', target: 'fixture-screen' }, { cwd, stateRoot })).toThrow(/pack-code/)
    })

    it('does not throw when pack-code is "enabled" (a stub config entry — pack-code has no real playbook yet)', () => {
      writeConfig({ 'pack-design': true, 'pack-code': true })

      expect(() => playbookStart({ name: 'design-to-code', target: 'fixture-screen' }, { cwd, stateRoot })).not.toThrow()
    })
  })
})
