...
export function runVrtWalker({
  stories,
  composeStories,
  committedBaselines
}: {
  stories: Record<string, any>
  composeStories: (storyModule: any) => Record<string, any>
  committedBaselines: unknown[] | Record<string, unknown> | null | undefined
}) {
      ...
      for (const [storyName, Story] of Object.entries(composed)) {
        for (const mode of ['light', 'dark']) {
          it(`${storyName} (${mode}) matches its committed baseline`, async () => {
            ...
            await (expect(container) as any).toMatchScreenshot(`${storyFile}/${storyName}.${mode}.png`)
          })
        }
      }
    })
