 ...
 * the caller pipes the MCP `get_metadata` output in as a normalized JSON
 * metadata tree (`{name, type, layoutMode?, children: [...]}`) via a file
 * arg or stdin.
 */
...
if (!screen || !wireframeNodeId || !figmaFileVersion) {
  console.error(
    'extract-region-contract: usage: node scripts/extract-region-contract.mjs <tree.json|-> <screen> <wireframeNodeId> <figmaFileVersion>'
  )
  process.exit(1)
}
