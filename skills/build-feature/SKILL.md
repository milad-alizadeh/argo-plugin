---
name: build-feature
description: Build out an EXISTING plan doc autonomously — runs the build-slices workflow (test-first build → verify → adversarial confirm → commit, slice by slice) isolated in its own git worktree, then hands off to the integrator. Use when a plan doc already exists and you want it built hands-off; for interactive single-slice TDD use /argo:test-first instead.
---
...
branch. It wraps the `build-slices` workflow with the isolation + integration lifecycle the
...
- **The workflow is installed:** `.claude/workflows/build-slices.*` must exist. If not,
...
`build-slices` workflow's spawned subagents inherit the worktree's cwd, so all building,
...
## 3. Run the build-slices workflow
Invoke the **`build-slices`** workflow (Workflow tool, `name: "build-slices"`) with
...
- **All slices DONE** → hand the branch to **`argo:integrator`** (the authoritative
  pre-merge gate; build-slices' per-slice checks are dev-time): it re-verifies, opens the
