...
**The design-rules audit is NOT a template installed into the host project**
(kit-extraction restructure). `figma-audit` bundles it on demand from
`@argohq/toolkit/design-kit/design-rules-audit` (+ the installed recipe's own
`design-kit/<recipe>/design-rules-walker` subpath) via `argo design
bundle-design-rules-audit` — see figma-audit/SKILL.md. Nothing audit-related is ever
...
| `spec-diff-walker/spec-diff.walker.spec-diff.js` | prefer `argo design emit-shims`; manual fill only for a bespoke shim | a THIN shim calling `runSpecDiffWalker` from `@argohq/toolkit/walkers`. Slots: `{{STORYBOOK_TEST_PACKAGE}}`/`{{STORIES_GLOB}}` as above; `{{DESIGN_SPECS_GLOB}}` ← glob of `design/specs/*.json` relative to the shim dir (`walkers.specsGlob`) — specs pair with stories by basename |
...
| (b) Structured user-config | the `design.<app>` block in `.claude/argo.json` | `mergeConfigShape` (from `@argohq/toolkit`) against the app's block: add missing shape keys, preserve every existing value, never delete on-disk-only keys; write `merged` via `JSON.stringify`, report `addedKeys`. |
