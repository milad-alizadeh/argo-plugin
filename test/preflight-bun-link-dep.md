# Preflight — bun link distribution for `@argohq/toolkit` (amended decision 4)
...
```
$ cd packages/toolkit && bun link
bun link v1.3.3 (274e01c7)
Success! Registered "@argohq/toolkit"
```
...
$ cat package.json
{"name":"link-consumer","private":true,"dependencies":{"@argohq/toolkit":"link:@argohq/toolkit"}}
$ bun install
+ @argohq/toolkit@link:@argohq/toolkit
1 package installed [8.00ms]
```

Assertions:

- **(a) symlink** — `node_modules/@argohq/toolkit` is a symlink to the linked
  repo's `packages/toolkit` (verified with `ls -l`).
- **(b) bin exposure** — `node_modules/.bin/argo -> ../@argohq/toolkit/bin/argo.js`.
- **(c) hooks.json wrapper path** — from the consumer dir, a benign PreToolUse
  ...
  `echo '{"tool_input":{"command":"ls"},"cwd":"/tmp"}' | npx --no @argohq/toolkit argo-hook bash-pretooluse` → exit 0.
...
2. **The link registration is per-machine, per-registering-checkout.** This
   ...
   re-register once from the main checkout (`cd <plugin repo>/packages/toolkit &&
...
3. **Release path** (documented, not fired here): `npm publish` from
   `packages/toolkit` via the Slice 7 OIDC workflow; consumers swap
   `"link:@argohq/toolkit"` → `"^<version>"`. No `vendor/` dir, no tarballs, no
